...
\mpar{这里我们用 Levi-Civita 符号来写转动生成元。参见\ref{eq:3.63}式下的文字。}%
...
\mpar{除了推动不变性导致的守恒量}%
会以这个或那个形式出现在每一个物理理论中。只要我们依旧有通常的时空对称性，动量、能量和角动量就是守恒量。在场论中，我们有两类对称性：一方面，拉格朗日量会在时空变换，如转动，下不变；另一方面，我们也会有在场自身的变换下的不变性，被称为{\bf 内禀对称性(internal symmetries)。}
...
。（无穷维）微分算符表示$x$变换到$x'$。这意味着利用这个表示我们能在不同的时空点或者在一个转动参考系中计算场分量。Lorentz 群的有限维变换使$\Psi$映射到$\Psi'$，即，混合场分量%
\mpar{请回忆这个例子，Weyl 旋量有两个分量而矢量场有四个分量。如果我们从一个不同的视角来考察矢量场$A_\mu=\begin{pmatrix} A_0\\ A_1\\ A_2\\ A_3 \end{pmatrix}$，即，从转动系看，它会形如$A'_\mu=\begin{pmatrix} A'_0\\ A'_1\\ A'_2\\ A'_3 \end{pmatrix}=\begin{pmatrix}{c} A_0\\ -A_2\\ A_1\\ A_3 \end{pmatrix}$。$A'_\mu$和$A_\mu$描述了从两个相差了绕$z$轴转动$\unit{90}{\degree}$的坐标系分别观察到的同一个场。}
...
\label{eq:4.30}
\end{equation}
其中$\epsilon_{\mu\nu}$为转动参数，$S_{\mu\nu}$为相应的有限维群表示中的转动算符，额外的负号仅为了方便。$S_{\mu\nu}$是由转动生成元$S_{i}=\frac{1}{2}\epsilon_{ijk}S_{jk}$和推动生成元$K_i=S_{i0}$构成，类似于\ref{eq:3.165}式中$M_{\mu\nu}$的定义。$S_{\mu\nu}$的这个定义使得我们同时进行旋转和推动。

第一项仅在旋转和推动时较重要，因为平移不会导致场分量的混合。推动所对应的守恒量依旧不是特别有意思，就像在粒子理论中得到的那样。所以实际上这一项只和转动相关。
...
\mpar{使用了$\partial_0 = \partial_t$和$\partial^i T_i^0=\nabla\cdot\vec{T}$，以及著名的散度定理$\int_V \rd^3 \bm{x}~\nabla A = \int_{\delta V} \rd^2 x~A$，即能让体积分化作面积分。对于散度定理，在那儿叫 Gauss 定理，有一个非常有启发性的证明，可以在\url{http://www.feynmanlectures.caltech.edu/II_03.html}上的第三章找到。Richard P. Feynman, Robert B. Leighton, and Matthew Sands. The Feynman Lectures on Physics: Volume 2. Addison-Wesley, 1st edition, 2 1977. ISBN 9780201021172（译注：中译本：《费恩曼物理学讲义》）}%求谁帮忙加一下再改一下格式
\begin{equation*}
\begin{aligned}
...
\mpar{因为对于任意$a_\mu$有$\partial_0 T_\mu^0 a_\mu = \partial_0 T_0^0 a_0 -\partial_0 T_i^0 a^i = 0$成立，所以我们对于每一个分量都能得到一个连续性方程。}%
，\ref{eq:4.39}式告诉我们它们是
\begin{eqnarray}
E &=& \int\rd^3\bm{x}~T_0^0 \\
P_i &=& \int\rd^3\bm{x}~T_i^0
\end{eqnarray}
其中$i=1,2,3$。它们分别被称为系统的总能量：因时间平移$x_0\rightarrow x_0+a_0$不变性而守恒；场的总动量：因空间平移$x_0\rightarrow x_0+a_0$不变性而守恒。
