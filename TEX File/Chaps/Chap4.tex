...
\mpar{我们使用的是这个公式的多变量形式，参见附录\ref{appendix2.3}：
\[
\begin{aligned}
...
\]
在最后一步中，我们用到了扰动$\delta q$在初末时刻\footnote{译注：右侧删去了原文的一对括号}$t_1,t_2$为零。由此可知，拉格朗日量的扰动$\delta {\mathcal L}$不必非要为零，而仅需满足一个更弱的条件
\begin{equation}
...
由于${\mathcal L}=\frac{1}{2}m{\dot\vec q}^2$不依赖于$\vec q$，故我们的拉格朗日量在空间平移变换$\vec{q}\rightarrow\vec{q}+\vec{a}$不变($\delta{\mathcal L}=0$)。这个对应的守恒量满足条件\ref{eq:4.13}，即$G=0$的情形\footnote{译注：求和约定，见第\ref{chap3}章边注129}
\begin{equation}
